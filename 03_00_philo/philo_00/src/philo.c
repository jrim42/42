...
/*   Updated: 2022/08/03 16:41:02 by jrim             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_philo(t_philo *philo, t_param *param, t_info *info)
{
	...
	pthread_mutex_init(info->is_done, NULL);
	philo = (t_philo *)malloc(sizeof(t_philo) * param->num_philo);
	info->fork = (pthread_mutex_t *)malloc(sizeof(pthread_mutex_t) * param->num_philo);
	if (philo == NULL || info->fork == NULL)
		print_error("cannot allocate memory", 12);
	idx = -1;
	while (++idx < param->num_philo)
	{
		...
		if (idx == 0)
			philo[idx].fork_left = &info->fork[param->num_philo - 1];
		else
			philo[idx].fork_left = &info->fork[idx - 1];
		philo[idx].fork_right = &info->fork[idx];
	}
	return (SUCCESS);
}
...
void	create_philo(t_info *info, t_philo *philo)
{
	...
	if (info->hungry_philo != info->param->num_philo)
	{
