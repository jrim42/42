...
/*   Updated: 2022/09/15 14:58:17 by jrim             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_scene *scene_init(void)
{
    ...
    world = object_init(PLN, plane_init(pt_init(0, -1, 0), vt_init(0, 1, 0)), rgb_init(0.5, 0.9, 0.5 ));
    obj_add(&world, object_init(PLN, plane_init(pt_init(0, 1, 0), vt_init(0, 1, 0)), rgb_init(0.2, 0.3, 0.3))); // world 에 구2 추가
    ...
    obj_add(&world, object_init(CYL, cyl_init(pt_init(-4, 0, -4), vt_init(0, 1, 0), 1, 2), rgb_init(0.6, 0.4, 0.1))); // world 에 구2 추가
    obj_add(&world, object_init(CYL, cyl_init(pt_init(-2, 0, -3), vt_init(0, 1, 0), 1, 2), rgb_init(0.6, 0.4, 0.1))); // world 에 구2 추가
    obj_add(&world, object_init(CYL, cyl_init(pt_init(0, 0, -2), vt_init(0, 1, 0), 1, 2), rgb_init(0.6, 0.4, 0.1))); // world 에 구2 추가
    obj_add(&world, object_init(CYL, cyl_init(pt_init(2, 0, -3), vt_init(0, 1, 0), 1, 2), rgb_init(0.6, 0.4, 0.1))); // world 에 구2 추가
    obj_add(&world, object_init(CYL, cyl_init(pt_init(4, 0, -4), vt_init(0, 1, 0), 1, 2), rgb_init(0.6, 0.4, 0.1))); // world 에 구2 추가
    // obj_add(&world, object_init(CYL, cyl_init(pt_init(-2, 1, -4), vt_init(-1, 1, 0), 2, 4), rgb_init(0.4, 0.6, 0.3))); // world 에 구2 추가
    scene->world = world;
    lights = object_init(LIGHT_PT, light_point(pt_init(2, 0.8, 0), rgb_init(1, 1, 1), 0.5), rgb_init(0, 0, 0)); // 더미 albedo
